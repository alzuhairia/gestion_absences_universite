...
if not DEBUG and os.getenv('TRUSTED_PROXY_CIDRS') is None:
    raise ImproperlyConfigured('TRUSTED_PROXY_CIDRS must be explicitly set when DEBUG=False.')
...
if not DEBUG and os.getenv('HEALTHCHECK_ALLOWLIST_CIDRS') is None:
    raise ImproperlyConfigured('HEALTHCHECK_ALLOWLIST_CIDRS must be explicitly set when DEBUG=False.')
HEALTHCHECK_ALLOWLIST_CIDRS = env_cidr_list('HEALTHCHECK_ALLOWLIST_CIDRS', '127.0.0.1/32,::1/128')
if not DEBUG and not HEALTHCHECK_ALLOWLIST_CIDRS:
    raise ImproperlyConfigured('HEALTHCHECK_ALLOWLIST_CIDRS must be set when DEBUG=False.')
