...
def create_postgres_trgm_indexes(apps, schema_editor):
    ...
    if not _pg_trgm_available(schema_editor):
        try:
            ...
            print(
                f"[accounts] pg_trgm extension unavailable, skipping trigram indexes: {exc}"
            )
            return
...
def drop_postgres_trgm_indexes(apps, schema_editor):
    ...
    schema_editor.execute(
        "DROP INDEX CONCURRENTLY IF EXISTS utilisateur_nom_trgm_gin_idx;"
    )
    schema_editor.execute(
        "DROP INDEX CONCURRENTLY IF EXISTS utilisateur_prenom_trgm_gin_idx;"
    )
    schema_editor.execute(
        "DROP INDEX CONCURRENTLY IF EXISTS utilisateur_email_trgm_gin_idx;"
    )
