...
def backfill_cours_required_fields(apps, schema_editor):
    ...
    for cours in (
        Cours.objects.using(db_alias).filter(id_annee__isnull=True).only("id_cours")
    ):
        year_ids = list(
        ...
        if len(year_ids) == 1:
            Cours.objects.using(db_alias).filter(id_cours=cours.id_cours).update(
                id_annee_id=year_ids[0]
            )
    ...
    for cours in (
        Cours.objects.using(db_alias).filter(niveau__isnull=True).only("id_cours")
    ):
        niveaux = list(
            Inscription.objects.using(db_alias)
            .filter(id_cours_id=cours.id_cours)
    ...
    operations = [
        ...
        migrations.AddConstraint(
            ...
            constraint=models.CheckConstraint(
                condition=models.Q(
                    ("nombre_total_periodes__gte", 1),
                    ("nombre_total_periodes__lte", 1000),
                ),
