...
def absence_details(request, id_inscription):
    ...
    for absence in absences:
        ...
        can_submit = (status == "NON JUSTIFIÃ‰E" or status == "EN ATTENTE") and not justification
...
def upload_justification(request, absence_id):
    ...
    if absence.id_inscription.id_etudiant != request.user:
        messages.error(request, "Acces non autorise. Vous ne pouvez consulter que vos propres absences.")
    ...
    if request.method == 'POST':
        ...
        with transaction.atomic():
            if justification and justification.state == 'REFUSEE':
                ...
                messages.error(request, "Impossible de soumettre le justificatif dans l'etat actuel.")
                return redirect('absences:upload', absence_id=absence_id)
...
def valider_justificatif(request, absence_id):
    ...
    # SECRETAIRE UNIQUEMENT - Les administrateurs ne gerent pas les justificatifs
    if request.user.role != User.Role.SECRETAIRE:
        if request.user.role == User.Role.ADMIN:
            messages.warning(
                request,
                "Cette fonction est reservee au secretariat. "
                "Les administrateurs gerent la configuration, pas les operations quotidiennes."
            )
        else:
    ...
    messages.success(request, f"L'absence de {absence.id_inscription.id_etudiant.email} a ete validee.")
    return redirect('dashboard:index')
...
def review_justification(request, absence_id):
    ...
    if request.user.role != User.Role.SECRETAIRE:
        if request.user.role == User.Role.ADMIN:
            ...
            messages.error(request, "Acces non autorise. Les justificatifs sont geres par le secretariat.")
        else:
            messages.error(request, "Action non autorisee.")
        return redirect('dashboard:index')
    ...
    if justification.document:
        ...
        document_name = f"justificatif_{absence.id_absence}{suffix}" if suffix else f"justificatif_{absence.id_absence}"
...
def mark_absence(request, course_id):
    ...
    inscriptions_qs = Inscription.objects.filter(id_cours=course).select_related('id_etudiant')
    ...
    if request.method == 'POST':
        ...
        if invalid_ids:
            ...
            raise PermissionDenied("Acces non autorise a une ou plusieurs inscriptions.")
        ...
        with transaction.atomic():
            ...
            if not created:
                 messages.info(
                     request, 
                     "Une sÃ©ance existe dÃ©jÃ  pour cette date. Les absences seront mises Ã  jour pour cette sÃ©ance existante."
                 )

            if not date_seance or not heure_debut or not heure_fin:
                messages.error(
                    request,
                    "Date et horaires de seance invalides. Veuillez verifier les champs saisis."
                )
            ...
            try:
                ...
                messages.error(
                    request,
                    "Format d'heure invalide. Veuillez utiliser le format HH:MM."
                )
                return redirect('absences:mark_absence', course_id=course_id)
            ...
            if t_fin <= t_debut:
                messages.error(
                    request,
                    "L'heure de fin doit etre strictement posterieure a l'heure de debut."
                )
                return redirect('absences:mark_absence', course_id=course_id)

            # Calcul duree theorique
            ...
            for key, value in request.POST.items():
                ...
                if status == 'ABSENT':
                    # Verifier si une absence existe deja pour cet etudiant et cette seance
                    if is_prof and existing_absence:
                        # Le professeur ne peut pas modifier une absence existante
                        # (il peut seulement creer de nouvelles absences)
                        continue

                    # PROTECTION SPECIALE : Absences justifiees par le secretariat
                    if is_prof and existing_absence and existing_absence.statut == 'JUSTIFIEE':
                        # L'absence est justifiee par le secretariat = OFFICIELLE
