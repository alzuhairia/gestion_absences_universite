...
def admin_faculties(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Facult̩ '{faculte.nom_faculte}' cr̩̩e avec succ̬s.")
...
def admin_faculty_edit(request, faculte_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Facult̩ '{faculte.nom_faculte}' {action} avec succ̬s.")
            return redirect('dashboard:admin_faculties')
    else:
...
def admin_faculty_delete(request, faculte_id):
    ...
    try:
        ...
        if cascade_info:
            success_msg += f" Suppression en cascade effectu̩e : {', '.join(cascade_info)}."
        ...
        logger.error(f"ProtectedError lors de la suppression de la facult̩ {faculte_nom}: {e}")
        messages.error(
            request,
        ...
        logger.error(f"Erreur lors de la suppression de la facult̩ {faculte_nom}: {e}", exc_info=True)
        messages.error(
            request,
...
def admin_departments(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"D̩partement '{dept.nom_departement}' cr̩̩ avec succ̬s.")
...
def admin_department_edit(request, dept_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"D̩partement '{dept.nom_departement}' {action} avec succ̬s.")
            return redirect('dashboard:admin_departments')
    else:
...
def admin_department_delete(request, dept_id):
    ...
    try:
        ...
        logger.error(f"ProtectedError lors de la suppression du d̩partement {dept_nom}: {e}")
        messages.error(
            request,
        ...
        logger.error(f"Erreur lors de la suppression du d̩partement {dept_nom}: {e}", exc_info=True)
        messages.error(
            request,
...
def admin_courses(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Cours '{cours.code_cours}' cr̩̩ avec succ̬s.")
...
def admin_course_edit(request, course_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Cours '{cours.code_cours}' {action} avec succ̬s.")
            return redirect('dashboard:admin_courses')
    else:
...
def admin_user_create(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Utilisateur '{user.email}' cr̩̩ avec succ̬s.")
...
def admin_user_edit(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Utilisateur '{user.email}' modifi̩ avec succ̬s.")
            return redirect('dashboard:admin_users')
    else:
    ...
    try:
        ...
        with transaction.atomic():
            ...
            locked_users = list(
                ...
                .filter(id_utilisateur__in=user_ids)
            )
            users_by_id = {user.id_utilisateur: user for user in locked_users}

            selected_user_ids = [
                uid for uid in users_by_id.keys()
                if uid != request.user.id_utilisateur
            ]
            ...
            if to_deactivate_ids:
                User.objects.filter(id_utilisateur__in=to_deactivate_ids).update(actif=False)
            ...
            for user_id in user_ids:
                try:
                    ...
                    try:
                        ...
                        User.objects.filter(id_utilisateur=user_id_for_log).update(actif=False)
                        log_action(
                            request.user,
                    ...
                    logger.exception("Erreur lors de la suppression de l'utilisateur %s", user_id)
                    errors.append("Erreur interne lors de la suppression de cet utilisateur.")
                    failed_count += 1

        if deleted_count > 0:
            messages.success(request, f"{deleted_count} utilisateur(s) supprime(s) avec succes.")
        if deactivated_count > 0:
            messages.warning(request, f"{deactivated_count} utilisateur(s) desactive(s) (dependances detectees).")
        if failed_count > 0:
...
def admin_user_delete(request, user_id):
    ...
    try:
        ...
        logger.exception("Exception lors de la recuperation de l'utilisateur %s", user_id)
        messages.error(
            request,
...
def admin_academic_years(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Ann̩e acad̩mique '{year.libelle}' cr̩̩e avec succ̬s.")
...
def admin_academic_year_set_active(request, year_id):
    ...
    messages.success(request, f"Ann̩e acad̩mique '{year.libelle}' d̩finie comme active.")
...
def admin_export_audit_csv(request):
    ...
    writer.writerow(['Date/Heure', 'Utilisateur', 'Email', 'R̫le', 'Action', 'Adresse IP'])
...
def get_prerequisites_by_level(request):
    ...
    course_id = request.GET.get('course_id', None)  # Pour exclure le cours actuel lors de l'̩dition
    ...
    try:
        ...
        if niveau == 1:
            ...
            return api_error('niveau invalide (doit ̻tre 1, 2 ou 3)', status=400, code='bad_request')
        ...
        logger.exception("Erreur API get_prerequisites_by_level [request_id=%s]", request_id)
        return api_error(
