"""
Gestion CRUD des facultés (rôle secrétaire).
...
def secretary_faculties(request):
    ...
    return render(
        ...
        {"faculties": faculties_page, "form": form},
    )
...
def secretary_faculty_delete(request, faculte_id):
    """Suppression d'une faculté avec suppression en cascade"""
    ...
    try:
        ...
        log_action(
            request.user,
            f"CRITIQUE: Suppression de la faculté '{faculte_nom}' (ID: {faculte_id}) - Gestion structure académique - Secrétaire",
            request,
            niveau="CRITIQUE",
            objet_type="FACULTE",
            objet_id=faculte_id,
        ...
        messages.error(request, f"Erreur lors de la suppression de la faculté '{faculte_nom}'.")
